...
Write-Host "  Crear index.php + .htaccess" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

# ============================================
# PASO 1: Subir create_index_php.php
# ============================================
Write-Host "[1] Subiendo create_index_php.php..." -ForegroundColor Yellow
...
try {
    ...
    $body = "op=trash&sourcefiles=$remoteDir/create_index_php.php"
...
try {
    $localFile = "create_index_php.php"
    ...
    $bodyLines = @()
    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"file-0`"; filename=`"create_index_php.php`""
...
# ============================================
# PASO 2: Ejecutar create_index_php.php
# ============================================
Write-Host ""
Write-Host "[2] Ejecutando create_index_php.php..." -ForegroundColor Yellow
Start-Sleep -Seconds 3
try {
    $r = Invoke-WebRequest -Uri "https://nexusmk.nexussolutionsyl.com/create_index_php.php" -UseBasicParsing -TimeoutSec 30
...
foreach ($test in $testUrls) {
    try {
        ...
        if ($code -eq 403) {
            Write-Host "  $($test.Desc): $code (denegado)" -ForegroundColor Green
        } else {

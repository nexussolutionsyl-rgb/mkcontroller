...
Write-Host "  Deploy Check LiteSpeed" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

# Subir check_litespeed.php
Write-Host "[1] Subiendo check_litespeed.php..." -ForegroundColor Yellow
...
try {
    ...
    $body = "op=trash&sourcefiles=$remoteDir/check_litespeed.php"
...
try {
    ...
    $bodyLines = @()
    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"file-0`"; filename=`"check_litespeed.php`""
...
# Ejecutar
Write-Host ""
Write-Host "[2] Ejecutando check_litespeed.php..." -ForegroundColor Yellow
Start-Sleep -Seconds 3
try {
    $r = Invoke-WebRequest -Uri "https://nexusmk.nexussolutionsyl.com/check_litespeed.php" -UseBasicParsing -TimeoutSec 30

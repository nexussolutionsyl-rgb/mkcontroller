...
Write-Host "=== Check .htaccess ===" -ForegroundColor Cyan
...
try {
    ...
    $body = "op=trash&sourcefiles=$remoteDir/check_htaccess.php"
...
try {
    ...
    $bodyLines = @()
    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"file-0`"; filename=`"check_htaccess.php`""
...
try {
    $r = Invoke-WebRequest -Uri "https://nexusmk.nexussolutionsyl.com/check_htaccess.php" -Method Get -UseBasicParsing -TimeoutSec 30
